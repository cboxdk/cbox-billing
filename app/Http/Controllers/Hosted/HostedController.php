...
abstract class HostedController extends Controller
{
    public function __construct(
        ...
        protected readonly EnvironmentRegistry $environments,
    ) {}
     ...
     * routes carry no credential to set the plane, so once the token resolves we set the
     * ambient {@see BillingContext} from the session's `environment` BEFORE any org / plan /
     * subscription / gateway query runs — a sandbox session then resolves and acts on ONLY its
     * own environment's data, and vice-versa.
     *
    ...
    protected function require(string $token, SessionType $type): BillingSession
    {
        ...
        $this->context->setEnvironment($this->environments->resolve($session->environmentKey()));
