    ...
    <div class="main">
        ...
        <header class="tb">
            ...
            <div style="display:flex;align-items:center;gap:6px">
                {{-- Environment switcher: pick the active billing plane. Production is the real,
                     protected plane; a sandbox is an isolated, disposable dataset. --}}
                @if (! empty($environments) && count($environments) > 1)
                    <form method="POST" action="{{ route('billing.environment.switch') }}" style="display:inline">
                        @csrf
                        <select name="environment" onchange="this.form.submit()" title="Active billing environment (plane)"
                                class="cbx-btn cbx-btn--sm cbx-btn--ghost @if(! empty($testMode)) cbx-pill--warning @endif" style="height:28px;padding:0 8px">
                            @foreach ($environments as $env)
                                <option value="{{ $env->key }}" @selected($activeEnvironment->key === $env->key)>
                                    {{ $env->name }}@if(! $env->isProduction()) (sandbox)@endif
                                </option>
                            @endforeach
                        </select>
                    </form>
